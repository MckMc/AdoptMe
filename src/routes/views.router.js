...
router.get('/home', async (_req, res) => {
  ...
  try {
    const [t, a, d] = await Promise.allSettled([
      withTimeout(PetModel.countDocuments({}).maxTimeMS(1000), 1200),
      withTimeout(PetModel.countDocuments({ adopted: true }).maxTimeMS(1000), 1200),
      withTimeout(PetModel.countDocuments({ adopted: false }).maxTimeMS(1000), 1200),
    ]);
